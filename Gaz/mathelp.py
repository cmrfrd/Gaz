...
def matsub(a, b):
    ...
    Subtract b from a
    '''
    assert len(a) == len(b), "Matrices do not have same X"
    assert len(a[0]) == len(b[0]), "Matrices do not have same Y"
...
def matmultconst(const, a):
    '''
    Multiply all the elements by a constant
    '''
...
def print_matrix(m,r):
    '''
    Print 2d list of integers, round each number
    '''
    X = deepcopy(m)
    for row in X:
        print [round(i,r) for i in row]
    print

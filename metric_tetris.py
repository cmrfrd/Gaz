...
def tetris_player_process(a, i, proc_id, total, **kwargs):
    """worker function that will play tetris in the background"""
    apps = []
    for x in range(i):
        App = tetris.TetrisApp(**kwargs)
        apps.append(App)
        print (total * proc_id) + x
        a[(total * proc_id) + x] = (App.run())#runs the game until it loses and returns the number of pieces 
    for App in apps:
        App.quit()
...
if __name__ == "__main__":
    ...
    for index in range(args.batch_size):
        check_records(dict_args, jobs)
        job = mult.Process(target=tetris_player_process, args=(lifetimes, jobs/args.batch_size, index, jobs/args.batch_size), kwargs=dict_args)
        job.start()
